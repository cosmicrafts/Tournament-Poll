...
actor Backend {
    ...
    public shared ({caller}) func registerUser() : async Bool {
        if (Array.indexOf<Principal>(caller, Array.map(users, func (user: { principal: Principal; username: Text; elo: Nat; avatarId: Nat }): Principal { user.principal }), func (u: Principal, p: Principal) : Bool { u == p }) == null) {
            let newUsers = Buffer.Buffer<{ principal: Principal; username: Text; elo: Nat; avatarId: Nat }>(users.size() + 1);
            for (user in users.vals()) {
                newUsers.add(user);
            };
            newUsers.add({principal = caller; username = ""; elo = 0; avatarId = 0});
            users := Buffer.toArray(newUsers);
            return true;
        };
        return false;
    };

    public shared ({caller}) func confirmParticipation(tournamentId: Nat) : async Bool {
        let userDetails = await fetchUserDetails(caller);
        users := Array.map(users, func (user: { principal: Principal; username: Text; elo: Nat; avatarId: Nat }): { principal: Principal; username: Text; elo: Nat; avatarId: Nat } {
            if (user.principal == caller) {
                {principal = caller; username = userDetails.username; elo = userDetails.elo; avatarId = userDetails.avatarId}
            } else {
                user
            }
        });
        // Mark user as confirmed for the specific tournament
        tournaments := Array.map(tournaments, func (tournament: Tournament) : Tournament {
            if (tournament.id == tournamentId) {
                ...
                { tournament with participants = Buffer.toArray(updatedParticipants) }
            } else {
                tournament
            }
        });
    ...
    public shared ({caller}) func resolveDispute(matchId: Nat, resolution: Text) : async Bool {
        if (caller != Principal.fromText("bdycp-b54e6-fvsng-ouies-a6zfm-khbnh-wcq3j-pv7qt-gywe2-em245-3ae")) {
            return false;
        };
    ...
    private func fetchUserDetails(_principal: Principal) : async {username: Text; elo: Nat; avatarId: Nat} {
        // Mock implementation for fetching user details from the main canister
        return {
            username = "Username";
            elo = 1200;
            avatarId = 1;
